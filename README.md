...
Scheduling rules live in `src/lib/scheduling/rules.ts`: address first, Calendar as source of truth when wired, travel = live drive + 15 minutes, never fake geography. The locked starter catalog in `src/lib/scheduling/services.ts` is Real Estate (Photography, Video, Aerial Photos, Zillow 360) and Construction (Photography, Video) — clients can pick more than one.
