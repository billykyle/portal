 ...
 *    When services are known, offered times and calendar event end use that
 *    sum instead of {@link DEFAULT_SLOT_MINUTES}. Aerial Photos is still the
 *    default until Billy locks a time.
 */
...
/** Fallback when no services are selected, and for Aerial Photos until Billy locks a time. */
